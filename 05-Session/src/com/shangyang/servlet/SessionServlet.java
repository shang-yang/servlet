 ...
 * 			该session对象的JSESSIONID使用Cookie技术存储到浏览器中，保证用户的
 * 			其他请求能够获取到同一个session对象，也就保证了不同请求能过获取到共享的数据
 * 		特点：
 * 			存储在服务器端
 * 			服务器进行创建
 * 			依赖Cookie技术
 ...
 * 				如果请求中没有session的标识符 也就是JSESSIONID，则创建新的session对象，并将其JESSIONID作为从cookie数据存储到浏览器内存中
 * 				如果session对象失效了，也会重新创建一个session对象，并将其JSESSIONID存储在浏览器内存中。
 * 			设置session存储时间
 * 				hs.setMaxInactiveInterval(int seconds);
 * 				注意：
 * 					在指定的时间内session对象没有被使用则销毁，如果使用了则重新记时
 * 			设置session强制失效
 * 				hs.invalidate();
 * 			存储和获取数据
 ...
 * 				注意：
 * 					存储的动作和取出的动作发生在不同的请求中，但是存储先于取出执行
 * 			使用时机：
 * 				一般用户在登录web项目时会将用户个人信息存储到Session中，供该用户的其他请求使用。
 * 			总结：
 * 				    session解决了一个用户的不同请求的数据共享问题，只要JSESSIONID不失效和session对象不失效的情况下，
