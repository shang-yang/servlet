...
<!-- 
	...
	extends : 设置jsp转译的java文件要继承的父类（包明 + 类明）
	作用：
		配置jsp文件的转译相关的参数
		
	Jsp的局部代码块
		特点：
			局部代码块中声明的java代码会被原样转译到jsp对应的servlet文件的_JspSevice方法中
			代码块中声明的变量都是局部变量
		使用：<% //<% java代码 % > %>
		缺点：
	...
	Jsp的静态引入和动态引入
		静态引入：
			...
			特点：
				...
				在网页中也就显示了合并后的显示效果
			注意：
				静态引入的jsp文件不会单独转译成 java（Servlet）文件。
	...
	Jsp的转发标签 forward
		...
		特点：
			...
			地址栏信息不变
		注意：
			在转发标签的两个标签中间除了写<% // <jsp:param value="value" name="name"/> %>子标签不会报错，其他任意字符都会报错。 	
	...
	Jsp的九大内置对象:
		...
		内容：
			...
			exception:		异常对象。存储了当前运行的异常信息。
						注意：使用此对象需要在page指定中使用属性 isErrorPage="true" 开启
			config: 		也就是ServletConfig，主要是用来获取 web.xml 中的配置数据，完成一些初始化数据的读取。
	...
	Jsp的路径
		...
		使用jsp中自带的全局路径声明：
			...
			<base href="<%= basePath %>">
			作用：
				给资源前面添加项目路径：		http://172.0.0.1:8080/虚拟项目名/
 -->
<!DOCTYPE html>
<html>
<head>
